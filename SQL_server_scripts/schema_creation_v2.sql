...
create table [tblEQUIPMENT_FACILITY]
	(
		...
		EquipmentSubtypeID int not null foreign key references [tblEQUIPMENT](EquipmentID),
		FacilityID int not null foreign key references [tblFACILITY](FacilityID)

		constraint pk_tblEQUIPMENT_FACILITY primary key (EqFacID),
		constraint fk_tblEQ_FAC_EquipmentSubtypeID foreign key (EquipmentSubtypeID) references [tblEQUIPMENT_SUBTYPE](EquipmentSubtypeID),
		constraint fk_tblEQ_FAC_FacilityID foreign key (FacilityID) references [tblFACILITY](FacilityID)
	)
